...
try:
    ...
    print("No DFPlayer created.")
    dffailed = True
else:
    dffailed = False
    dftimer = Timer()
    dftimer.set_duration(10)

i2c = board.I2C()  # uses board.SCL and board.SDA
#icm = adafruit_icm20x.ICM20X(i2c, 0x69)
try:
...
vs_output3 = Vspeed(init_position=0, result="int")
vs_output3.set_bounds(lower_bound=-1, upper_bound=255)
vs_output4 = Vspeed(init_position=0, result="int")
vs_output4.set_bounds(lower_bound=-1, upper_bound=255)
vs_output5 = Vspeed(init_position=0, result="int")
vs_output5.set_bounds(lower_bound=-1, upper_bound=255)
...
# status_light = digitalio.DigitalInOut(board.LED)
# status_light.direction = digitalio.Direction.OUTPUT
...
# secrets = {"ssid": "emotion", "password": "supersecret"}
secrets = {"ssid": "animal", "password": "supersecret"}
wifi = wifimanager.ESPSPI_WiFiManager(esp, secrets) #,status_light
wifi.create_ap()
...
#     print((x,y, z))
    ...
    if(len(mylist) >= 5):
        ...
        if mylist[0] > (average + 0.8) or mylist[0] < (average - 0.8):
            position_output3, running_output3, changed_output3 = vs_output3.sequence(sequence=placement_sequence, loop_max=0)
            if position_output3 >= 0:
                pixels.fill((0, position_output3, 0))
                pixels.show()
                isPeak = True
                print("Peak detected ") #+ str(i) )
                #i = i+1
                if(dffailed == False and playing == False):
                    dfplayer.play(track=2)
                    dftimer.start()
                    playing = True
                if(dffailed == False and playing == True and dftimer.expired()):
                    playing = False
            else:
                mylist.pop(0)
                mylist.insert(0,z)
        ...
        print("List building in progress...")
        mylist.insert(0,z)
    ...
    if isPeak == False:
        ...
        if timer.expired():
            ...
            position_output2, running_output2, changed_output2 = vs_output4.sequence(sequence=gradient_sequence2, loop_max=0)
            position_output4, running_output4, changed_output4 = vs_output5.sequence(sequence=gradient_sequence3, loop_max=0)
            pixels.fill((position_output1, position_output2, position_output4))
            pixels.show()
